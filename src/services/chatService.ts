 ...
 * Retrieves only relevant chunks from vector database based on user query
 */
async function buildContextWithRAG(
  ...
  try {
    ...
    const relevantChunks = await ragService.retrieveContext(userMessage, projectId, 5, 0.7);
    ...
    console.log(`[ChatService] 📉 Token savings: Using ${relevantChunks.length} chunks instead of full dataset`);
...
async function callOpenAI(messages: ChatMessage[]): Promise<string> {
  ...
  const validation = validateOpenAIConfig(ENV.OPENAI_API_KEY);
  if (!validation.valid) {
  ...
  if (!tokenCheck.isValid) {
    ...
    console.warn(`[ChatService] ${tokenCheck.warning}`);
  }

  // Log token usage
  console.log(`[ChatService] Estimated tokens: ${tokenCheck.estimate.promptTokens} prompt + ${tokenCheck.estimate.maxResponseTokens} response = ${tokenCheck.estimate.totalTokens} total`);
...
async function saveConversation(
  ...
  if (conversationId) {
    ...
    (conversation as any).updateLastMessage(userMessage);
    await conversation.save();
  } else {
    // Create new conversation
    const title = (ChatConversation as any).generateTitle(userMessage);
