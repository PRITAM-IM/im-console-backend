 ...
 * Now supports enhanced context with increased chunk retrieval
 */
export async function retrieveContext(
  ...
  try {
    ...
    // Step 2: Query vector database with enhanced retrieval
    ...
    console.log(`✅ Retrieved ${results.length} relevant chunks with enhanced context`);
    return results;
  } catch (error: any) {
...
/**
 * Retrieve context with historical data from multiple time periods
 * Includes current period and last month data for comparative analysis
 */
export async function retrieveContextWithHistory(
  userQuery: string,
  projectId: string,
  ...
  try {
    ...
    const periodLength = Math.abs(new Date(currentEndDate).getTime() - currentStart.getTime());
    const historyEndDate = new Date(currentStart.getTime() - 1).toISOString().split('T')[0];
    const historyStartDate = new Date(currentStart.getTime() - periodLength - 86400000).toISOString().split('T')[0];

    // Query for historical period data
    const historicalResults = await vectorService.queryVectors({
      embedding: queryEmbedding,
      projectId,
      topK: Math.floor(topK * 0.3), // 30% for historical context
      minScore: minScore - 0.1, // Slightly lower threshold for historical data
      dateRange: {
    ...
    console.log(`✅ Retrieved ${currentResults.length} current + ${historicalResults.length} historical chunks`);
    return {
      currentPeriod: currentResults,
      historicalPeriod: historicalResults,
    };
  } catch (error: any) {
 ...
 * Enhanced with historical context awareness
 */
export function buildContextFromChunks(chunks: VectorQueryResult[], label?: string): string {
...
export function buildContextWithHistory(
  currentChunks: VectorQueryResult[],
  historicalChunks: VectorQueryResult[]
): string {
  let context = '';

  // Add current period context
  if (currentChunks.length > 0) {
    context += buildContextFromChunks(currentChunks, '=== CURRENT PERIOD DATA ===');
  }

  // Add historical period context for comparison
  if (historicalChunks.length > 0) {
    context += '\n=== PREVIOUS PERIOD DATA (For Comparison) ===\n\n';
    context += buildContextFromChunks(historicalChunks, '');
    context += '\nNote: Use this historical data to identify trends, compare performance, and provide month-over-month insights.\n';
  }

  if (!currentChunks.length && !historicalChunks.length) {
    return 'No relevant data found in the vector database.';
  }
